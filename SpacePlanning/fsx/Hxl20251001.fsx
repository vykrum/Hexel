...
module Parse =
    ...
    let spaceCxl
        ...
        let bdryVtx 
            ...
            match Regex.IsMatch(input, @"^[0-9,\-]+$") with
            | false -> [||], 0, 0
            | true ->
                let groups =
                    input.Split('-')
                    |> Array.choose (fun part ->
                        ...
                        match pairs with
                        | [||] -> None
                        | ps when ps.Length < 3 -> None
                        | ps -> Some ps)

                // Flatten all tuples to compute bounds
                let allPairs = groups |> Array.collect id
                match allPairs with
                | [||] -> groups, 0, 0
                | ps ->
                    let xs = ps |> Array.map fst
                    let ys = ps |> Array.map snd
                    let w = Array.max xs - Array.min xs
                    let h = Array.max ys - Array.min ys
                    groups, w, h
        ...
        // Boundary and Islands
        let bdV1 = match spcAt1 |> Map.tryFind "B" with 
                    | Some a -> 
                                let vtx,_,_ = bdryVtx a
                                vtx
                    | None -> [||]
...
let bdryVtx (input: string) : (int*int)[][] =
    match Regex.IsMatch(input, @"^[0-9,\-]+$") with
    ...
    | true ->
        ...
        |> Array.choose (fun part ->
            ...
            match pairs with
            | [||] -> None                     // drop empty
            | ps when ps.Length < 3 -> None    // must have ≥ 3 pairs
            | ps -> Some ps
        )
