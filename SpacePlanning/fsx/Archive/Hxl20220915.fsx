...
let inc (hxl : (int * Hxl)[]) (occ : Hxl[]) = 
    ...
    let hx4 = match (hx3|>Array.length) > 2 with 
                    | false -> hx3
                    | true -> 
                                let hx41 = hx3 |> Array.removeAt ((hx3|>Array.length)-1)
                                let hx42 = hx1 |> Array.last |> adj
                                let hx43 = exc ([|oc1;hx41|] |> Array.concat) hx42
                                let hx44 = hx43 |> Array.tryHead
                                match hx44 with 
                                | Some a -> hx41 |> Array.append [|a|]
                                | None -> hx41 |> Array.append [|hx1 |> Array.last|]
...
// Testing
let Loc1 hxl = Array.map (fun x -> x.Loc) hxl
let oc1 = (adj {Avl=6 ; Loc=(0,1,0)})
let occ = oc1 |> Array.map(fun x -> chk x oc1) 
let a = Array.zip [|1..6|] occ[1..6] 
let b = (nui a occ) 
        //|> Array.map (Array.map(fun y ->(fun x -> x.Loc)y))