...
let nstdCxlsWrp
    ...
    let txPsn (crd : (int*int)[][]) =
        let x1 = crd |> Array.map(fun y -> Array.map(fun (x,_) -> double x)y)|> Array.map(fun x -> int(Array.average x))
        let y1 = crd |> Array.map(fun y -> Array.map(fun (_,x) -> double x)y)|> Array.map(fun x -> int(Array.average x))
        Array.map2 (fun x y -> x,y) x1 y1
    txPsn crd
